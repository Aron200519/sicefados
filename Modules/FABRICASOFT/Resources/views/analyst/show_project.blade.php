        ...
        <div class="card-body">
            ...
            <div class="mt-4">
                ...
                @if($progressPercentage == 100)
                    <!-- NOTA: Solo el administrador puede entregar el proyecto -->
                    <div class="mt-3 text-center">
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle me-2"></i>
                            <strong>¡PROYECTO COMPLETADO!</strong> Todas las fases han sido finalizadas exitosamente. 
                            <br><small class="text-muted">Contacta al administrador para la entrega final del proyecto.</small>
                        </div>
                    </div>
                @endif
            </div>
    ...
    <div class="modal fade" id="modalAgregarInformacion{{ $completedPhase->id }}" tabindex="-1" aria-labelledby="modalAgregarInformacionLabel{{ $completedPhase->id }}" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <form id="formAgregarInformacion{{ $completedPhase->id }}" action="{{ route('fabricasoft.analyst.projects.phase.info.add.specific', $project->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
// Función para abrir el modal de agregar información
function abrirModalAgregarInformacion(phaseId) {
    const modal = document.getElementById(`modalAgregarInformacion${phaseId}`);
    if (modal) {
...
// Función para abrir el modal de completar fase
function abrirModalCompletarFase(phaseId) {
...
// Función para editar información de fase
function editarInformacionFase(historyId) {
    // Aquí puedes implementar la lógica para cargar los datos en el modal de edición
...
// Función para eliminar información de fase
function eliminarInformacionFase(historyId) {
    if (confirm('¿Estás seguro de que quieres eliminar esta información? Esta acción no se puede deshacer.')) {
        ...
        console.log('Eliminando información con ID:', historyId);
    }
}
...
function handleCompletarFaseSubmit(event) {
    ...
    const url = `{{ route('fabricasoft.analyst.projects.phase.complete.with.info', ['projectId' => $project->id, 'phaseId' => ':phaseId']) }}`.replace(':phaseId', phaseId);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Event listeners para los formularios específicos de cada fase
    @foreach($project->phases->where('status', 'completed') as $completedPhase)
        ...
        if (formAgregarInformacion{{ $completedPhase->id }}) {
            ...
            formAgregarInformacion{{ $completedPhase->id }}.addEventListener('submit', function(e) {
                e.preventDefault();
                console.log('Formulario de fase {{ $completedPhase->id }} enviado');
                
                const formData = new FormData(this);
                
                // Log de los datos del formulario
                console.log('Datos del formulario de fase {{ $completedPhase->id }}:');
                for (let [key, value] of formData.entries()) {
                    console.log(key + ': ' + value);
                }
                ...
                console.log('Enviando formulario de fase {{ $completedPhase->id }} a:', this.action);
                ...
                fetch(this.action, {
                    ...
                    if (data.success) {
                        alert('Información agregada exitosamente a la fase {{ $completedPhase->phase_name }}.');
